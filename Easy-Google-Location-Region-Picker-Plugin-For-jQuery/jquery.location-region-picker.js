...
(function ($, window, document, undefined) {
	...
	LocationRegionPicker.prototype.newLocationSelected = function () {
		...
		if (this.place.geometry && this.place.geometry.location) {
			$details.append('Lat: <input type="text" class="form-control" id="latitude" value="' + this.place.geometry.location.lat() + '" disabled />');
			$details.append('Lon: <input type="text" class="form-control" id="longitude" value="' + this.place.geometry.location.lng() + '" disabled />');

		}
		var weatherLat = this.place.geometry.location.lat();
		var weatherLong = this.place.geometry.location.lng();
		console.log(weatherLat, weatherLong);

		this.$element.after($details);

		this.updateMapView();
		// return weatherLat, weatherLong;
