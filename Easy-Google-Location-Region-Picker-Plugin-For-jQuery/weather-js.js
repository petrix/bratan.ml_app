...
function mainWeather(weatherLat, weatherLong) {
    ...
    // var longi = 30.2034;
    var data;
    var iconvalue = ['cloudy', 'snow', 'partly-cloudy-day', 'partly-cloudy-night', 'fog', 'clear-day', 'wind'];
    var iconimage = ['<div class="icon cloudy"></div>', '<div class="icon snow"></div>', '<div class="icon partly-cloudy-day"></div>', '<div class="icon partly-cloudy-night"></div>', '<div class="icon fog"></div>', '<div class="icon clear-day"></div>', '<div class="icon wind"></div>'];
    var iconimagebig = ['<div class="iconbig cloudy"></div>', '<div class="iconbig snow"></div>', '<div class="iconbig partly-cloudy-day"></div>', '<div class="iconbig partly-cloudy-night"></div>', '<div class="iconbig fog"></div>', '<div class="iconbig clear-day"></div>', '<div class="iconbig wind"></div>'];
    moment.locale('uk');
    $.getJSON(url + apiKey + "/" + weatherLat + "," + weatherLong + "?units=uk&lang=uk&callback=?", function (data) {
        ...
        for (var i = 0; i < 24; i++) {
            ...
            $(".sumhr1").append('<div class="' + classDayNight + ' ' + timeclass + '">' + '<article>' + '<b>' + time + '</b>' + '<p>' + 'wind:' + '</p>' + '<i class="fas fa-long-arrow-alt-up" style="transform: rotate(' + data.hourly.data[i].windBearing + 'deg)"></i>' + '<article><b>' + data.hourly.data[i].windSpeed.toFixed(1) + '</b>' + '<p>' + 'M/s' + '</p>' + '</article>' + '<i class="fas fa-tint"></i>' + '<b>' + humidity + '%' + '</b>' + '</article>' + '<article>' + mainIcon + '<p>' + 'temp:' + '</p>' + '<b>' + temp + "&deg;" + '</b>' + '<article style="height:' + tempheight * 10 + 'px;' + 'background-color:' + tempcolor + '">' + '</article>' + '</article>' + '</div>');
        }
        ...
        for (i = 24; i < 48; i++) {
            ...
            $(".sumhr2").append('<div class="' + classDayNight + ' ' + timeclass + '">' + '<article>' + '<b>' + time + '</b>' + '<p>' + 'wind:' + '</p>' + '<i class="fas fa-long-arrow-alt-up" style="transform: rotate(' + data.hourly.data[i].windBearing + 'deg)"></i>' + '<article><b>' + data.hourly.data[i].windSpeed.toFixed(1) + '</b>' + '<p>' + 'M/s' + '</p>' + '</article>' + '<i class="fas fa-tint"></i>' + '<b>' + humidity + '%' + '</b>' + '</article>' + '<article>' + mainIcon + '<p>' + 'temp:' + '</p>' + '<b>' + temp + "&deg;" + '</b>' + '<article style="height:' + tempheight * 10 + 'px;' + 'background-color:' + tempcolor + '">' + '</article>' + '</article>' + '</div>');
        }
        ...
        $("#sumdaily").html(data.daily.summary);
    });
    // });
